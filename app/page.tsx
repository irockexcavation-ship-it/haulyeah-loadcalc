...
const initialQuarries: Quarry[] = [
  {
    ...
    name: "Mulzer",
    materials: [
      { id: "m1", name: "DGA", pricePerTon: 18.95, taxable: true },
...
const makeLine = (quarries: Quarry[] = initialQuarries): MaterialLine => ({
  id: `line-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
  quarryId: quarries[0].id,
  materialId: quarries[0].materials[0].id,
  tons: "20",
  priceOverride: "",
});
...
export default function HaulYeahLoadCalcApp() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        if (Array.isArray(data.quarries) && data.quarries.length > 0) {
  ...
  const updateMaterial = (quarryId: string, materialId: string, patch: Partial<Material>) => {
    setQuarries((prev) =>
      prev.map((q) =>
        q.id === quarryId
          ? {
  ...
  const clearForNewCustomer = () => {
    ...
    setMaterialLines([makeLine(quarries)]);
  ...
  const resetSavedData = () => {
    ...
    setMaterialLines([makeLine(initialQuarries)]);
    setTimeout(() => {
      customerInputRef.current?.focus();
    }, 50);
  };
  ...
  const calc = useMemo(() => {
    ...
    const totalLoads = Number(truckCapacity) > 0 ? Math.ceil(totalTons / (Number(truckCapacity) || 1)) : 0;
          ...
          <TabsContent value="calculator" className="space-y-4">
            ...
            <Card className="rounded-3xl border-zinc-800 bg-zinc-900/80 shadow-2xl shadow-black/20">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="text-sm text-zinc-400">
                  Using saved truck capacity:{" "}
                  <span className="text-zinc-200">{truckCapacity} tons</span>
                </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => updateMaterialLine(line.id, { priceOverride: e.target.value })}
                          inputMode="decimal"
                          className="mt-2 rounded-2xl border-zinc-700 bg-zinc-950 text-white"
                        />
                      </div>
